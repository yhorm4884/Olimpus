...
const Notifications = ({ userId }) => {
  ...
  useEffect(() => {
    axios.get(`http://backend.olimpus.arkania.es/notifications/list/${userId}/`)
      .then(response => {
  ...
  const handleUpdateNotification = (action) => {
    axios.post(`http://backend.olimpus.arkania.es/notifications/update/${selectedNotification.id}/`, { action })
      .then(response => {
...
const ActivityList = ({ userId }) => {
  ...
  const fetchActivities = async () => {
    try {
      const response = await axios.get(`http://backend.olimpus.arkania.es/activities/actividades/${userId}`);
  ...
  const handleSaveActivity = async ({ activityData, userId, isEdit = false }) => {
    const endpoint = isEdit
      ? `http://backend.olimpus.arkania.es/activities/edit_actividad/${currentActivity.id}/`
      : `http://backend.olimpus.arkania.es/activities/add_actividad/${userId}/`;
  ...
  const handleDeleteSelected = () => {
    if (selectedActivities.length > 0) {
      axios.post(`http://backend.olimpus.arkania.es/activities/delete/`, { ids: selectedActivities })
        .then(response => {
  ...
  const handleDeleteActivity = async (activityId) => {
    try {
        const response = await axios.post(`http://backend.olimpus.arkania.es/activities/delete_single/`, { id: activityId });
        alert(response.data.message);
  ...
  const fetchEmpresas = () => {
    axios.get('http://backend.olimpus.arkania.es/api/empresas/')
      .then(response => {
